...
RC DynamicThread::run() {
	...
	// #RAIN: 当 SEED != 0 时，用确定性种子初始化，使 DYNAMIC 模式下 batch_id 切换序列可复现。
	// 当 SEED == 0 时，用时间戳，保证每次测试的切换序列真随机。
#if SEED != 0
	default_random_engine generator((uint64_t)SEED + (uint64_t)g_node_id * 7919UL + _thd_id * 6271UL);
#else
	default_random_engine generator((unsigned)get_sys_clock());
#endif
