...
int main(int argc, char *argv[]) {
	...
	printf("Initializing workset predictor... ");
	workset_predictor.init(g_k_value, g_min_support, g_hash_lock_num, false);
	printf("Done\n");

	// #RAIN 导入预测器模型（如已启用）
	if (g_predictor_import_enable) {
		printf("Importing workset predictor model from '%s'... ", g_predictor_import_file);
		fflush(stdout);
		if (workset_predictor.import_from_csv(g_predictor_import_file)) {
			printf("Done (imported)\n");
		} else {
			printf("Failed (will continue with empty predictor)\n");
		}
	}

	printf("Initializing active ws table... ");
	fflush(stdout);
	active_ws_table.init(g_hash_lock_num);
	printf("Done\n");
	...
	*/
	...
	printf("\n");
	fflush(stdout);
	// #RAIN: 打印关键指标摘要（在原始统计之后追加，不影响原有输出）
	if (STATS_ENABLE) stats.print_rain_summary();

	// #RAIN 导出预测器模型（如已启用）
#if CC_ALG == HDCC
	if (g_predictor_export_enable) {
		printf("Exporting workset predictor model to '%s'... ", g_predictor_export_file);
		fflush(stdout);
		if (workset_predictor.export_to_csv(g_predictor_export_file)) {
			printf("Done\n");
		} else {
			printf("Failed\n");
		}
