...
void 			TxnStats::inc_op_step() {
	op_step += 1;
}				

uint32_t 	TxnStats::get_op_step() const{
	return op_step;
}		

#if CC_ALG == HDCC
/*
 * 将真实 row 映射成 ActiveWsTable / WorksetPredictor 使用的统一 key。
 *
 * YCSB:
 *   row primary key 本身即可。
 *
 * TPCC:
 *   执行期拿到的是实际 row，因此用 table_id 放到高位，避免不同表主键重叠。
 *   注意：这和 Sequencer 里的 TPCCTableKey offset 方案语义一致，都是“区分表”。
 *   如果你希望训练/预测键空间完全一致，建议二选一统一：
 *     1) 都使用 table_id 高位编码；
 *     2) 都使用 TPCCTableKey offset。
 */
static inline uint64_t txn_ws_key(row_t *row) {
#if WORKLOAD == TPCC
    uint64_t table_id = row->get_table()->get_table_id();
    return (table_id << 56) | (row->get_primary_key() & 0x00ffffffffffffffUL);
#else
    return row->get_primary_key();
#endif
}

static inline WsMode access_to_ws_mode(access_t type) {
    return type == WR ? WS_WR : WS_RD;
}

static inline bool has_predict_conflict(const std::vector<WsConflictHit> &hits) {
    for (auto &hit : hits) {
...
/*
 * 从 TxnManager 当前实际访问集收集上下文。
 */
static inline void collect_actual_ws(TxnManager *txn_man,
                                     std::vector<uint64_t> &keys,
                                     std::vector<WsItem> &items) {
    keys.clear();
    items.clear();

    uint64_t access_cnt = txn_man->get_access_cnt();
    keys.reserve(access_cnt);
    items.reserve(access_cnt);

    for (uint64_t i = 0; i < access_cnt; i++) {
        row_t *row = txn_man->get_access_original_row(i);
        if (!row) {
            continue;
        }

        uint64_t key = txn_ws_key(row);

        WsItem item;
        item.key = key;
        item.mode = access_to_ws_mode(txn_man->get_access_type(i));
        item.src = WS_ACTUAL;
        item.conf = 1.0;
 ...
 * 方案二核心：
 *
 * 当前事务已经按 SILO 执行到第 k 步；
 * 此时用“真实已访问数据项”作为上下文预测后续数据项；
 * 然后用 “actual_items + pred_items” 与 ActiveWsTable 中其他活跃事务做冲突检测。
 */
static inline bool trigger_k_step_prediction(TxnManager *txn_man) {
    std::vector<uint64_t> actual_keys;
    std::vector<WsItem> actual_items;

    collect_actual_ws(txn_man, actual_keys, actual_items);

    if (actual_keys.empty()) {
        return false;
    }

    std::vector<WorksetPredictor::PredItem> preds =
        workset_predictor.predict(actual_keys, g_predict_topk);

    std::vector<WsItem> candidate = actual_items;

    for (auto &pred : preds) {
        WsItem item;
        item.key = pred.item;
        item.mode = WS_UNK;
        item.src = WS_PREDICTED;
        item.conf = pred.prob;
        candidate.push_back(item);
    }

    active_ws_table.register_items(txn_man->get_txn_id(), candidate);

    std::vector<WsConflictHit> hits =
        active_ws_table.detect_conflicts(txn_man->get_txn_id(), candidate);

    return has_predict_conflict(hits);
}
...
void TxnManager::init(uint64_t thd_id, Workload * h_wl) {
  ...
  // write_set = (int *) mem_allocator.alloc(sizeof(int) * 100);
	...
	/* ------------------------- #RAIN predict ------------------------- */
	predict_checked 		= 	false;
	retry_as_calvin 		= 	false;
	predict_access_cnt 	= 	0;
	/* ------------------------- #RAIN end ------------------------- */
...
/*
	* 只在 SILO 路径下做方案二：
	* - Calvin 路径本身是确定性执行，不做 k-step 中途升级。
	* - 每次真实访问都把 actual key 写入 active_ws_table。
	*/
if (algo == SILO) {
		uint64_t key = txn_ws_key(row);
		active_ws_table.upsert_actual(get_txn_id(), key, access_to_ws_mode(type));

		if (g_predict_mode == 2 && !predict_checked) {
				predict_access_cnt++;

				if (predict_access_cnt >= g_k_value) {
						predict_checked = true;

						if (trigger_k_step_prediction(this)) {
								retry_as_calvin = true;
						}
				}
		}
}
...
/* ------------------------- #RAIN end ------------------------- */
  ...
  INC_STATS(get_thd_id(), trans_store_access_count, 1);
	...
	#if CC_ALG == HDCC
    /*
     * 高风险时不做“无缝切 Calvin”，而是返回 Abort，
     * 让已有 SILO abort/restart 机制接管。
     */
    if (g_predict_mode == 2 && retry_as_calvin) {
        return Abort;
    }
	#endif
