...
static inline uint64_t worker_ws_key(row_t *row) {
#if WORKLOAD == TPCC
...
/*
 * 提交后用真实工作集训练 predictor。
 * 这样比在 Sequencer 阶段训练更准确，尤其 TPCC 分支执行路径可能变化。
 */
static inline void observe_committed_workset(TxnManager *txn_man) {
    ...
    for (uint64_t i = 0; i < access_cnt; i++) {
        ...
        if (row) {
            keys.push_back(worker_ws_key(row));
        }
    }

    workset_predictor.observe(keys);
}
...
void WorkerThread::commit() {
  ...
  /*
   * 提交成功：
   * 1. 用真实访问序列训练 predictor；
   * 2. 从 active_ws_table 删除，避免幽灵冲突。
   */
  observe_committed_workset(txn_man);
  active_ws_table.remove_txn(txn_man->get_txn_id());
#endif
...
void WorkerThread::abort() {
  ...
  #if CC_ALG == HDCC
  /*
   * 方案二：在 reset() 之前保存 retry_as_calvin 标记。
   * reset() 会把该字段清零，abort_queue.enqueue 在 reset() 之后调用，
   * 因此必须用局部变量保存，再通过 txn_man->retry_as_calvin 恢复传给 enqueue。
   */
  bool saved_retry_as_calvin = txn_man->retry_as_calvin;

  /*
   * abort 也必须清 active pool，否则会留下幽灵冲突。
   */
  active_ws_table.remove_txn(txn_man->get_txn_id());
  #endif
...
/* ------------------------- #RAIN end ------------------------- */
  ...
  #if CC_ALG == HDCC
  /*
   * reset() 已将 retry_as_calvin 清零，这里恢复保存的值，
   * 使下方 abort_queue.enqueue(txn_man) 能读到正确标记。
   */
  txn_man->retry_as_calvin = saved_retry_as_calvin;
  #endif
  /* ------------------------- #RAIN end ------------------------- */
