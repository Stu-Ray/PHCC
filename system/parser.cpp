...
void print_usage() {
  ...
  printf("\t-tcINT       ; CALVIN_THREAD_CNT (FOR MIXCC)\n");
	...
	printf("\t-lsleINT     ; load statement log enable (0/1)\n");
	printf("\t-lslf STRING ; load statement log file path\n");
	printf("\t-lslsINT     ; load statement log sample ratio (N, >=1)\n");
	...
	printf("  [#RAIN predict]:\n");
	printf("\t-rkINT       ; K_VALUE (predictor prefix context length)\n");
	printf("\t-rtopkINT    ; PREDICT_TOPK (max predicted items returned)\n");
	printf("\t-rriskFLOAT  ; PREDICT_RISK_THRESHOLD (conflict risk threshold, e.g. 0.3)\n");
	printf("\t-rmsINT      ; MIN_PREDICT_SUPPORT (min sample count for prediction)\n");
	printf("\t-rimp0|1     ; import predictor model on startup (0=off, 1=on)\n");
	printf("\t-rexp0|1     ; export predictor model on exit (0=off, 1=on)\n");
	printf("\t-rif STRING  ; predictor import file full path\n");
	printf("\t-ref STRING  ; predictor export file full path\n");
...
void parser(int argc, char * argv[]) {
    ...
    if (argv[i][1] == 'n' && argv[i][2] == 'd' && argv[i][3] == 'l' && argv[i][4] == 'y')
      ...
      g_num_wh = atoi( &argv[i][3] );
			...
			txn_file = argv[++i];
    else if (argv[i][1] == 'l' && argv[i][2] == 's' && argv[i][3] == 'l' && argv[i][4] == 'e')
      g_load_stmt_log_enable = atoi(&argv[i][5]) == 1;
    else if (argv[i][1] == 'l' && argv[i][2] == 's' && argv[i][3] == 'l' && argv[i][4] == 'f')
      g_load_stmt_log_file = argv[++i];
    else if (argv[i][1] == 'l' && argv[i][2] == 's' && argv[i][3] == 'l' && argv[i][4] == 's')
      g_load_stmt_log_sample = atoi(&argv[i][5]) > 0 ? atoi(&argv[i][5]) : 1;
    ...
    // #RAIN predict parameters
    // -rkINT : K_VALUE
    else if (argv[i][1] == 'r' && argv[i][2] == 'k')
      g_k_value = atoi( &argv[i][3] );
    // -rtopkINT : PREDICT_TOPK
    else if (argv[i][1] == 'r' && argv[i][2] == 't' && argv[i][3] == 'o' && argv[i][4] == 'p' && argv[i][5] == 'k')
      g_predict_topk = atoi( &argv[i][6] );
    // -rriskFLOAT : PREDICT_RISK_THRESHOLD
    else if (argv[i][1] == 'r' && argv[i][2] == 'r' && argv[i][3] == 'i' && argv[i][4] == 's' && argv[i][5] == 'k')
      g_predict_risk_threshold = atof( &argv[i][6] );
    // -rmsINT : MIN_PREDICT_SUPPORT
    else if (argv[i][1] == 'r' && argv[i][2] == 'm' && argv[i][3] == 's')
      g_min_support = atoi( &argv[i][4] );
    // -rimpINT : predictor import enable (0/1)
    else if (argv[i][1] == 'r' && argv[i][2] == 'i' && argv[i][3] == 'm' && argv[i][4] == 'p')
      g_predictor_import_enable = atoi( &argv[i][5] ) == 1;
    // -rexpINT : predictor export enable (0/1)
    else if (argv[i][1] == 'r' && argv[i][2] == 'e' && argv[i][3] == 'x' && argv[i][4] == 'p')
      g_predictor_export_enable = atoi( &argv[i][5] ) == 1;
    // -rif STRING : predictor import file path
    else if (argv[i][1] == 'r' && argv[i][2] == 'i' && argv[i][3] == 'f')
      g_predictor_import_file = argv[++i];
    // -ref STRING : predictor export file path
    else if (argv[i][1] == 'r' && argv[i][2] == 'e' && argv[i][3] == 'f')
      g_predictor_export_file = argv[++i];
		else if (argv[i][1] == 'h') {
			print_usage();
			exit(0);
		} else {
  ...
  g_max_supplier_key *= g_node_cnt;
    ...
    //g_inflight_max = g_inflight_max / g_node_cnt;
      ...
      printf("g_load_stmt_log_enable %d\n", g_load_stmt_log_enable);
      printf("g_load_stmt_log_file %s\n", g_load_stmt_log_file);
      printf("g_load_stmt_log_sample %u\n", g_load_stmt_log_sample);
#if CC_ALG == HDCC
      printf("g_calvin_thread_cnt %d\n",g_calvin_thread_cnt);
      // #RAIN predict parameters
      printf("[#RAIN] g_predict_mode %u\n", g_predict_mode);
      printf("[#RAIN] g_k_value %u\n", g_k_value);
      printf("[#RAIN] g_predict_topk %u\n", g_predict_topk);
      printf("[#RAIN] g_predict_risk_threshold %f\n", g_predict_risk_threshold);
      printf("[#RAIN] g_min_support %u\n", g_min_support);
      printf("[#RAIN] g_predictor_import_enable %d\n", (int)g_predictor_import_enable);
      printf("[#RAIN] g_predictor_export_enable %d\n", (int)g_predictor_export_enable);
      printf("[#RAIN] g_predictor_import_file %s\n", g_predictor_import_file);
      printf("[#RAIN] g_predictor_export_file %s\n", g_predictor_export_file);
#endif
