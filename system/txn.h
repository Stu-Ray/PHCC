...
public:
	...
	void 			inc_op_step(); 					// #RAIN  增加步数
	uint32_t 	get_op_step() const;		// #RAIN  获取步数
	uint32_t 	op_step;  							// #RAIN  当前事务执行到的步数
...
public:
  ...
  int*        	read_set;
	...
	/*
	* #RAIN 方案二执行期预测状态：
		* predict_checked: 当前事务是否已经触发过 k-step 预测，避免同一事务反复检测。
		* retry_as_calvin:
		*   若 k-step 检测发现高风险，则置 true。
		*   WorkerThread::abort() 会保留该标记，AbortQueue 会把重试消息标记为 CALVIN。
		* predict_access_cnt: SILO 路径下已记录的实际访问步数。
	*/
	bool        predict_checked;
	bool        retry_as_calvin;
	uint64_t    predict_access_cnt;
