...
// #RAIN: 服务端关键指标摘要输出
// ============================================================
void Stats::print_rain_summary() {
  if (!STATS_ENABLE) return;

  // 确保 totals 已汇总（print() 之后调用时已汇总，此处保险再做一次）
  totals->clear();
  for (uint64_t i = 0; i < thd_cnt; i++) totals->combine(_stats[i]);
  ...
  // ---- 基础计算 ----
  double runtime_s  = t->total_runtime / BILLION;
  double tput       = (runtime_s > 0) ? (double)t->txn_cnt / runtime_s : 0.0;
  double commit_cnt = (double)t->total_txn_commit_cnt;
  double abort_cnt  = (double)t->total_txn_abort_cnt;
  double total_fin  = commit_cnt + abort_cnt;
  double commit_rate = (total_fin > 0) ? commit_cnt / total_fin * 100.0 : 0.0;
  double abort_rate  = (total_fin > 0) ? abort_cnt  / total_fin * 100.0 : 0.0;

  // ---- 延迟分位数（last_start_commit_latency，单位 s）----
  double lat_p50 = 0, lat_p95 = 0, lat_p99 = 0, lat_avg = 0;
  // 汇总所有线程的延迟数组（totals->last_start_commit_latency 已在 combine 时合并）
  StatsArr & lat = t->last_start_commit_latency;
  if (lat.cnt > 0) {
    lat.quicksort(0, lat.cnt - 1);
    lat_avg = (double)lat.get_avg()          / BILLION;
    lat_p50 = (double)lat.get_percentile(50) / BILLION;
    lat_p95 = (double)lat.get_percentile(95) / BILLION;
    lat_p99 = (double)lat.get_percentile(99) / BILLION;
  }

  // ---- 网络 ----
  double net_send_mb = (double)(t->msg_batch_size_bytes_to_server + t->msg_batch_size_bytes_to_client) / 1024.0 / 1024.0;

  // ---- ASCII 表格输出 ----
  const char * LINE = "+-------------------------------------------+----------------------+\n";
  const char * HDR  = "| %-41s | %-20s |\n";
  const char * ROW  = "| %-41s | %-20s |\n";
  char vbuf[64];

  printf("\n");
  printf("%s", LINE);
  printf(HDR, "  #RAIN SERVER SUMMARY", "Value");
  printf("%s", LINE);

  // 吞吐量
  snprintf(vbuf, sizeof(vbuf), "%.1f txn/s", tput);
  printf(ROW, "  Throughput", vbuf);

  // 运行时间
  snprintf(vbuf, sizeof(vbuf), "%.3f s", runtime_s);
  printf(ROW, "  Runtime", vbuf);

  printf("%s", LINE);

  // 事务计数
  snprintf(vbuf, sizeof(vbuf), "%lu", t->txn_cnt);
  printf(ROW, "  Total Txn Executed", vbuf);

  snprintf(vbuf, sizeof(vbuf), "%lu  (%.1f%%)", t->total_txn_commit_cnt, commit_rate);
  printf(ROW, "  Committed", vbuf);

  snprintf(vbuf, sizeof(vbuf), "%lu  (%.1f%%)", t->total_txn_abort_cnt, abort_rate);
  printf(ROW, "  Aborted", vbuf);

  printf("%s", LINE);

  // 延迟
  if (lat.cnt > 0) {
    snprintf(vbuf, sizeof(vbuf), "%.4f s", lat_avg);
    printf(ROW, "  Latency Avg (last-start->commit)", vbuf);

    snprintf(vbuf, sizeof(vbuf), "%.4f s", lat_p50);
    printf(ROW, "  Latency P50", vbuf);

    snprintf(vbuf, sizeof(vbuf), "%.4f s", lat_p95);
    printf(ROW, "  Latency P95", vbuf);

    snprintf(vbuf, sizeof(vbuf), "%.4f s", lat_p99);
    printf(ROW, "  Latency P99", vbuf);
  } else {
    printf(ROW, "  Latency", "(no data)");
  }
  ...
  // HDCC 路由
#if CC_ALG == HDCC
  // ---- HDCC 路由统计（仅在 HDCC 模式下声明，避免非 HDCC 编译时 unused-variable 错误）----
  uint64_t silo_cnt     = t->hdcc_silo_cnt;
  uint64_t calvin_cnt   = t->hdcc_calvin_cnt;
  uint64_t silo_loc     = t->hdcc_silo_local_cnt;
  uint64_t calvin_loc   = t->hdcc_calvin_local_cnt;
  uint64_t total_routed = silo_cnt + calvin_cnt;
  double   silo_pct     = (total_routed > 0) ? (double)silo_cnt   / total_routed * 100.0 : 0.0;
  double   calvin_pct   = (total_routed > 0) ? (double)calvin_cnt / total_routed * 100.0 : 0.0;
  uint64_t saved_cnt    = t->saved_txn_cnt;

  snprintf(vbuf, sizeof(vbuf), "%lu  (%.1f%%)", silo_cnt, silo_pct);
  printf(ROW, "  Routed -> Silo (OCC)", vbuf);

  snprintf(vbuf, sizeof(vbuf), "  local %lu", silo_loc);
  printf(ROW, "    of which local-only", vbuf);

  snprintf(vbuf, sizeof(vbuf), "%lu  (%.1f%%)", calvin_cnt, calvin_pct);
  printf(ROW, "  Routed -> Calvin (Det.)", vbuf);

  snprintf(vbuf, sizeof(vbuf), "  local %lu", calvin_loc);
  printf(ROW, "    of which local-only", vbuf);

  snprintf(vbuf, sizeof(vbuf), "%lu", saved_cnt);
  printf(ROW, "  Saved by Predictor", vbuf);

  // 确定性中止
  snprintf(vbuf, sizeof(vbuf), "%lu / %lu",
           t->deterministic_abort_cnt_silo, t->deterministic_abort_cnt_calvin);
  printf(ROW, "  Det.Abort Silo / Calvin", vbuf);
#endif

  printf("%s", LINE);

  // 网络
  snprintf(vbuf, sizeof(vbuf), "%lu / %lu", t->msg_send_cnt, t->msg_recv_cnt);
  printf(ROW, "  Msg Send / Recv", vbuf);

  snprintf(vbuf, sizeof(vbuf), "%.2f MB", net_send_mb);
  printf(ROW, "  Total Bytes Transferred", vbuf);
...
// #RAIN: 客户端关键指标摘要输出
// ============================================================
void Stats::print_rain_client_summary() {
  if (!STATS_ENABLE) return;
  ...
  // 客户端端到端延迟（client_client_latency）
  double ccl_avg = 0, ccl_p50 = 0, ccl_p95 = 0, ccl_p99 = 0;
  StatsArr & ccl = t->client_client_latency;
  if (ccl.cnt > 0) {
    ccl.quicksort(0, ccl.cnt - 1);
    ccl_avg = (double)ccl.get_avg()          / BILLION;
    ccl_p50 = (double)ccl.get_percentile(50) / BILLION;
    ccl_p95 = (double)ccl.get_percentile(95) / BILLION;
    ccl_p99 = (double)ccl.get_percentile(99) / BILLION;
  }
  ...
  printf("\n");
  printf("%s", LINE);
  printf(HDR, "  #RAIN CLIENT SUMMARY", "Value");
  printf("%s", LINE);

  snprintf(vbuf, sizeof(vbuf), "%.1f txn/s", tput);
  printf(ROW, "  Client Throughput", vbuf);

  snprintf(vbuf, sizeof(vbuf), "%.3f s", runtime_s);
  printf(ROW, "  Runtime", vbuf);

  snprintf(vbuf, sizeof(vbuf), "%lu", t->txn_cnt);
  printf(ROW, "  Txn Completed", vbuf);

  snprintf(vbuf, sizeof(vbuf), "%lu", t->txn_sent_cnt);
  printf(ROW, "  Txn Sent", vbuf);

  printf("%s", LINE);

  if (ccl.cnt > 0) {
    snprintf(vbuf, sizeof(vbuf), "%.4f s", ccl_avg);
    printf(ROW, "  E2E Latency Avg", vbuf);

    snprintf(vbuf, sizeof(vbuf), "%.4f s", ccl_p50);
    printf(ROW, "  E2E Latency P50", vbuf);

    snprintf(vbuf, sizeof(vbuf), "%.4f s", ccl_p95);
    printf(ROW, "  E2E Latency P95", vbuf);

    snprintf(vbuf, sizeof(vbuf), "%.4f s", ccl_p99);
    printf(ROW, "  E2E Latency P99", vbuf);
  } else {
    printf(ROW, "  E2E Latency", "(no data)");
  }
  ...
  double bytes_mb = (double)(t->msg_batch_size_bytes_to_server + t->msg_batch_size_bytes_to_client) / 1024.0 / 1024.0;
  snprintf(vbuf, sizeof(vbuf), "%.2f MB", bytes_mb);
  printf(ROW, "  Total Bytes Transferred", vbuf);
