...
#define   LOAD_STMT_LOG_ENABLE        1                                     // 日志开关（0关闭，1开启）
#define   LOAD_STMT_LOG_FILE          "/opt/HDCC/log/load_statements.log"   // 日志文件输出路径
#define   LOAD_STMT_LOG_SAMPLE        1                                     // 日志采样间隔（每隔多少个操作采样一次）
#define K_VALUE 3
#define PREDICT_TOPK 5
// 预测模式：
//   0 -- 关闭 RAIN 预测（纯 cc_selector 路由）
//   1 -- 方案一（pre-exec predict）：Sequencer 入口处从 query 消息静态提取前 k 个 key，
//        送入 WorksetPredictor 预测剩余项，用于冲突检测，TPCC DELIVERY/ORDER_STATUS 仅粗粒度兜底
//   2 -- 方案二（k-step predict）：SILO 执行到第 k 步后，用真实已访问集触发预测，冲突则 abort → Calvin
//   3 -- 方案三（pre-exec full workset）：Sequencer 入口处提取完整工作集，直接用于精确冲突检测，
//        不依赖 WorksetPredictor，不截断为前 k 项。
//        适用范围：TPCC 仅 PAYMENT / NEW_ORDER（工作集完全由 query 参数静态确定，100% 精确）。
//        若扩展到 TPCC_ALL（含 DELIVERY/ORDER_STATUS），需恢复 dry-run 补充逻辑（见 git log）。
#define PREDICT_MODE 3
#define PREDICT_RISK_THREHOLD 0.3
#define MIN_PREDICT_SUPPORT 1
#define   HASH_LOCK_NUM               64                                    // 预测器内哈希分片锁数量

// #RAIN WorksetPredictor 导入导出配置
#define PREDICTOR_IMPORT_ENABLE 0
#define PREDICTOR_EXPORT_ENABLE 0
#define   PREDICTOR_IMPORT_FILE       "/opt/HDCC/model/predictor.csv"      // 导入文件完整路径
#define   PREDICTOR_EXPORT_FILE       "/opt/HDCC/export/predictor.csv"      // 导出文件完整路径

/* ------------------------- #RAIN end ------------------------- */

// quick setup
#define FIN_BY_TIME true
#define MAX_TXN_IN_FLIGHT 500
