...
BaseQuery * DAQueryGenerator::create_query(Workload * h_wl, uint64_t home_partition_id)
{
  ...
  const auto handle = [this](ActionSequence &&act_seq,uint64_t seq_id) {
    ...
    for(size_t i=0;i<seq_size;i++)
    {
        ...
        os << "workload=DA node=" << g_node_id
           << " seq_id=" << DAQ_t->seq_id
           << " stmt_idx=" << i
           << " trans_id=" << DAQ_t->trans_id
           << " item_id=" << DAQ_t->item_id
           << " txn_type=" << DAQ_t->txn_type
           << " state=" << DAQ_t->state
           << " next_state=" << DAQ_t->next_state
           << " last_state=" << DAQ_t->last_state
           << " write_version=" << DAQ_t->write_version;
        load_stmt_log_append(os.str());
