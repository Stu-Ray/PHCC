...
// #RAIN: per-generator versions using myrand (for deterministic seed support)
// These use a caller-supplied myrand instead of the global rand() state.
// NURand_r requires the caller to pre-compute C constants in init() and pass them.
class myrand;
