...
void YCSBQueryGenerator::init() {
	...
	// #RAIN: 当 SEED != 0 时，使用 (SEED + node_id + 每次 init 的递增计数) 作为确定性种子，
	// 保证两次测试生成完全相同的 YCSB 事务集合。
	// 当 SEED == 0 时，退化为时间戳（原始行为）。
#if SEED != 0
	static std::atomic<uint64_t> s_ycsb_gen_counter(0);
	uint64_t my_count = s_ycsb_gen_counter.fetch_add(1, std::memory_order_relaxed);
	uint64_t mrand_seed = (uint64_t)SEED + (uint64_t)g_node_id * 1000003UL + my_count * 999983UL;
