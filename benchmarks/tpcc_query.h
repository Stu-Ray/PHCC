...
public:
  // #RAIN: init() allocates and seeds mrand (mirrors YCSBQueryGenerator::init()).
  // Must be called before the first create_query() when SEED != 0.
  // When SEED == 0 the global rand() path is used unchanged.
  void init();
...
private:
	...
	myrand * mrand;
	// #RAIN: per-generator NURand constants (initialised once in init()).
	// These replace the file-scope statics in tpcc_helper.cpp::NURand().
	uint64_t _nur_c_255;
	uint64_t _nur_c_1023;
	uint64_t _nur_c_8191;
};
